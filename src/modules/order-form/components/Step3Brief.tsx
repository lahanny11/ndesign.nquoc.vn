...
export default function Step3Brief({ data, onChange, draftOrderId }: Props) {
  ...
  function commitHex() {
    if (!isValidHex(hexInput)) { setHexError(hexInput.replace(/^#/, '').length > 0); return }
    const normalized = normalizeHex(hexInput)
    if (!data.primary_colors.includes(normalized) && data.primary_colors.length < 5) {
      onChange({ primary_colors: [...data.primary_colors, normalized] })
    }
    setHexInput('')
    setHexError(false)
  }
          ...
          <div>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#6E6E73', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                    Màu đã chọn · {data.primary_colors.length}/5
                  </span>
                  <button
                    type="button"
                    onClick={() => onChange({ primary_colors: [] })}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
              {COLOR_PRESETS.map(({ hex, name }) => {
                ...
                const isSelected = data.primary_colors.some(c => c.toUpperCase() === normalized)
                return (
                  <button
                    key={hex}
                    type="button"
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'stretch' }}>
                ...
                <div style={{
                  ...
                  <input
                    ...
                    onChange={e => { const val = '#' + e.target.value; setHexInput(val); setHexError(e.target.value.length > 0 && !isValidHex(val)) }}
                  ...
                  />
                  {hexError && (
                    <span style={{ fontSize: 10, color: '#FF3B30', flexShrink: 0 }}>Không hợp lệ</span>
                  )}
                </div>
                {/* Add button */}
                <button
                  type="button"
                  onClick={commitHex}
                  disabled={!isValidHex(hexInput) || data.primary_colors.length >= 5}
                  style={{
                    width: 42, height: 42, borderRadius: 10, border: 'none', flexShrink: 0,
                    background: isValidHex(hexInput) && data.primary_colors.length < 5 ? '#1D1D1F' : 'rgba(0,0,0,0.08)',
                    color: isValidHex(hexInput) && data.primary_colors.length < 5 ? '#fff' : '#AEAEB2',
                    cursor: isValidHex(hexInput) && data.primary_colors.length < 5 ? 'pointer' : 'not-allowed',
